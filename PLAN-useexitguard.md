...
> **계약: 라우트당 useExitGuard 1개(codex R11·R12 [high])**. passive 폴백은 owner 소유권 뒤집힘을 막는 **런타임 안전망**일 뿐, **중첩된 dirty 가드(같은 화면에 자기 `when`/`onConfirmExit`를 가진 2개 이상)는 지원 패턴이 아니다.** 그런 구성은 자식 가드가 조용히 무력화돼 데이터 유실로 이어질 수 있으므로: **DEV 모드(빌드 호환 감지 — 리터럴 `import.meta.env.DEV` 금지, tsc `types:[react,react-dom]`에서 typecheck 실패. 권장: `const isDev = (import.meta as ImportMeta & { env?: { DEV?: boolean } }).env?.DEV === true`. SDD SC-9가 유일 허용 형태)에서 같은 entry에 다른 활성 uid 마커가 있는데 내 `when===true`이면 `console.error`로 "useExitGuard: 라우트당 1개만 — 두 번째 가드의 보호가 비활성화됨" 경고**(조용한 무력화 금지, codex "passive must not be silent"). 슬라이스 1 AC가 각 세션 라우트에 정확히 1개만 마운트되도록 보장. (StrictMode 이중 마운트는 같은 컴포넌트라 cleanup→remount 순서로 1개만 활성 — uid 재생성 + 마운트당 idempotent로 흡수.)
...
8. **누수 한정(정직한 계약)** — "잔여 sentinel 0"은 **명시적 release 경로(confirmExit/releaseAndNavigate)**에서만 보장. 그 외(일반 disarm·언마운트·생 navigate 미조정 이탈)는 stale sentinel 1개가 남을 수 있음(History API는 중간/임의 entry 삭제 불가 + 훅이 blind back()을 안 하므로 — codex R10). 이는 **router-agnostic popstate 방식의 수용된 한계(ADR-5)**. 단 **bounded(통제된 remount 한정)**: ① idempotent 재arm으로 마운트당 sentinel 최대 1개(무한 중복 금지) ② **guard 화면이 실제로 remount되는 경우에 한해** — stale entry는 가드 화면과 동일 URL이라 브라우저 Back으로 착지 시 그 화면 remount + (when=true면)재arm되어 phantom listener-less 상태 없음 ③ when=false로 disarm된 stale entry는 같은 URL 중복 entry 1개(무해). ⚠️ **redirect/auth-gated/route-loader로 guard 화면이 remount되지 않는 no-remount raw-navigate 경로는 Slice 0가 보장하지 못함**(라우터 무관 훅·앱 라우트 무변경) — 이는 **Slice 1 blocking 인벤토리**에서 releaseAndNavigate 또는 명시 confirm/useBlocker로 커버(SDD SC-T22·한계절·ADR-5와 동일). → **슬라이스 1 AC**: 데이터 유실 위험 경로는 releaseAndNavigate 경유(잔여 0). 통제 가능한 비위험 bounded stale만 허용.
...
| **미조정 생 navigate→목적지서 브라우저 Back (guard 화면 remount되는 통제 케이스 한정)** | stale entry(가드 URL) 착지 후 **remount되는 경우에 한해** 재arm(listener 부착), sentinel 마운트당 ≤1(무한 중복 0), phantom listener-less 없음. ⚠️ no-remount(redirect/auth/loader) 경로는 Slice 0 미보장→Slice 1 인벤토리(ADR-5) |
